...
if [ "$temp" -lt "5" ]; then echo "fail to print all command line flags"; fail=$((fail+1)); else echo "ok"; fi
...
if [ ! -z "$temp" ]; then
    ...
    temp=`valgrind --log-fd=1 "$MCX" --bench cube60b -n 1e4`
...
#temp=`which compute-sanitizer 2> /dev/null`
#if [ ! -z "$temp" ]; then
#    echo "test gpu memory errors using compute-sanitizer ... "
#    temp=`compute-sanitizer "$MCX" --bench cube60b -n 1e5`
#    haserror=`echo $temp | grep -o -E 'MCX[A-Z]* ERROR.-'`
