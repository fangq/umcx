...
- **Acknowledgement:** This project is part of the [NeuroJSON project](https://neurojson.org)
  supported by US National Institute of Health (NIH)
  grant [U24-NS124027](https://reporter.nih.gov/project-details/10308329)

[![CI](https://github.com/NeuroJSON/zmat/actions/workflows/run_test.yml/badge.svg)](https://github.com/NeuroJSON/zmat/actions/workflows/run_test.yml)
[![Linux Wheel](https://github.com/NeuroJSON/zmat/actions/workflows/build_linux_wheel.yml/badge.svg)](https://github.com/NeuroJSON/zmat/actions/workflows/build_linux_wheel.yml)
[![macOS Wheel](https://github.com/NeuroJSON/zmat/actions/workflows/build_macos_wheel.yml/badge.svg)](https://github.com/NeuroJSON/zmat/actions/workflows/build_macos_wheel.yml)
[![Windows Wheel](https://github.com/NeuroJSON/zmat/actions/workflows/build_windows_wheel.yml/badge.svg)](https://github.com/NeuroJSON/zmat/actions/workflows/build_windows_wheel.yml)
...
- [Installing ZMat on Linux Distributions](#installing-zmat-on-linux-distributions)
- [Installing ZMat for Python](#installing-zmat-for-python)
- [Using ZMat in Python](#using-zmat-in-python)
- [Using ZMat in MATLAB](#using-zmat-in-matlab)
- [Compile ZMat](#compile-zmat)
...
- **Updated bundled libraries**:
  - [C-Blosc2](https://blosc.org) updated to v2.23.1
  - [Zstandard](https://facebook.github.io/zstd/) updated to v1.5.7
  - [miniz](https://github.com/richgel999/miniz) updated to v3.1.1
  - [LZ4](https://lz4.github.io/lz4/) updated to v1.10.0
...
- **Multi-threaded lzip and XZ** — lzip now compresses chunks in parallel across
  multiple threads; XZ uses block-level multi-threading via the LZMA2 encoder with
  automatic block-size selection for efficient parallelism on any input size.

- **Memory and safety fixes** — multiple safety, memory-leak, and efficiency
  issues in `zmatlib.c` have been resolved, including overflow guards before
  `realloc`, dead-code removal, proper error returns replacing `assert()`, and
...
called `blosc2` (https://blosc.org). The `blosc2` compressor is not a single
...
int zmat_run(
      ...
      const int zipid,            /* 0: zlib, 1: gzip, 2: base64, 3: lzip, 4: lzma, 5: lz4, 6: lz4hc,
                                     7: zstd, 8: blosc2blosclz, 9: blosc2lz4, 10: blosc2lz4hc,
                                     11: blosc2zlib, 12: blosc2zstd, 13: xz */
...
number of threads and byte-shuffling length, via the `flags.param` interface
in the following data structure, and pass `flags.iscompress` as the last
input to `zmat_run`:
...
For Octave, one needs to copy the `zipmat.mex` file inside the `"octave"`,
...
to enable the [relevant PPA](https://launchpad.net/~fangq/+archive/ubuntu/ppa)
(personal package archive) first.
...
or decompressing/decoding data streams. The help info of the function is shown
below.
...
input:
    ...
    options: a series of ('name', value) pairs, supported options include
            'nthread': number of threads (default 4); used by lzip, xz, zstd, and blosc2 meta-compressors
...
output:
     output: a uint8 row vector, storing the compressed or decompressed data;
            ...
            struct, the original data type and dimensions are restored.
     info: (optional) a struct storing additional info regarding the input data, may have
           ...
           'level': a copy of the iscompress flag; if non-zero, specifying compression
                   level, see above
           'shuffle'/'typesize': present when wrapper-level byte-shuffle was applied
           'matrixtype': present for special types (sparse, diagonal, permutation, range)
...
**Method 1:** Open MATLAB or Octave, and run the below commands:
...
- If you have MATLAB R2017b or later, you may skip this step.
  To compile zmat in MATLAB R2017a or earlier on Windows, you must
...
**Method 3:** Open a terminal, and run the below shell commands:
...
or browsing the github site at https://github.com/NeuroJSON/zmat
...
[NeuroJSON forum](https://github.com/orgs/NeuroJSON/discussions)
...
6. **ZStd**: https://facebook.github.io/zstd/
   ...
   - License: BSD 3-Clause License (http://www.opensource.org/licenses/bsd-license.php)
7. **base64_{encode,decode} functions** in zmat.cpp:
